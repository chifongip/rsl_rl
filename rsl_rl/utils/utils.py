...
class RunningMeanStd:
    """Calculates the running mean and variance of a data stream.
...
class Normalizer(RunningMeanStd):
    ...
    def normalize_torch(self, x: torch.Tensor, device: str | torch.device) -> torch.Tensor:
        ...
        if device != self._cached_device or self._cached_mean is None:
            ...
            self._cached_std = torch.sqrt(self.var + self.epsilon).to(device=device, dtype=torch.float32)
        return torch.clamp((x - self._cached_mean) / self._cached_std, -self.clip_obs, self.clip_obs)
